...
async def pnl_daily() -> list[float]:
    container = get_app_container()
    pnl = await container.execution_bridge.track_pnl()
    return [pnl['total_return'] / 5 for _ in range(5)]


@router.post('/orders/cancel')
async def orders_cancel(order_id: str = '') -> dict[str, object]:
    container = get_app_container()
    result = await container.execution_bridge.cancel(order_id)
